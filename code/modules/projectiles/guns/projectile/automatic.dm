...
/obj/item/gun/projectile/automatic/rifle/adhomian
	...
	max_shells = 0 //This was 25, but allowed the gun to be loaded to 50 rounds, a better solution would add the bullets to the magazine
...
/obj/item/gun/projectile/automatic/rifle/adhomian/attackby(obj/item/attacking_item, mob/user)
	if(istype(attacking_item, /obj/item/ammo_magazine/boltaction) && !ammo_magazine)
		...
		return
	if (istype(attacking_item, /obj/item/ammo_magazine/boltaction) && ammo_magazine)
		to_chat(user, SPAN_WARNING("\The [src] has had its loading gate is blocked by the magazine, it cannot be reloaded with a stripper clip!"))
		return
	..()
