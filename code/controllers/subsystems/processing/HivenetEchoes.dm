...
#define GETPROJECTION "<i><span class='game say'>Hivenet, <span class='name'>a [pick("Faint", "Distant", "Fading", "Fleeting", "Drifting", "Low", "Weak", "Far", "Pinging", "Whispering")] Echo</span> projects <span class='vaurca'>[pick(echo_projections)]</span></span></i>"
#define ECHOMSG(input) "<i><span class='game say'>Hivenet, <span class='name'>a [pick("Faint", "Distant", "Fading", "Fleeting", "Drifting", "Low", "Weak", "Far", "Pinging", "Whispering")] Echo</span> broadcasts, <span class='vaurca'>\"[pick(input)]\"</span></span></i>"
#define CHECK_VALID_SOCKET if(player.internal_organs_by_name[BP_NEURAL_SOCKET] && !within_jamming_range(player) && (GLOB.all_languages[LANGUAGE_VAURCA] in player.languages) && (!player.sleeping) && (player.stat != DEAD))
#define CHECKPLAYER for(var/mob/living/carbon/human/player in GLOB.player_list)
///////////
/* OUTLINE: Echoes are on by default through "catchecho", some sectors (3 atm) have it off automatically at roundstart.
They trigger through components, at /datums/components/_hivenetechoes.dm. All Vaurcae get a HiveEchoes component after spawning.
That checks the time and calls the two echo procs when the cooldown is up. Broadcasts & projections have different timers to not feel synced up:
180-300s for former, 240-480s for latter. "islive" is a carried argument from HiveEchoes that makes echoes generate only with living vaurcae.

GetBroadcastEcho() proc manages most things:
It gives messages for when echoes are disabled, decides the broadcast topic, if they'll be joined, sets starter & response strings with a switch
block, then sends both.

GetProjectionEcho() proc is much simpler, it just directly picks a projection to send.
...
/proc/GetProjectionEcho(islive) //Gets called at /datums/components/_hivenetechoes.dm. No living Vaurcae = no echoes.
...
//120 out-of-context, projected sensations. The wackier projections are probably from the Aether
	var/static/list/echo_projections = list(
		...
		"the texture of a roasted diona nymph.", "the taste of a [prob(50)?"cooked":"bland"] vaurca carapace.", "eating [prob(50)?"cooked":"fried"] chicken.",
		...
		"remorselessly executing something living.", "hatching [prob(50)?"again":"once more"].", "being slapped.", "flexing headtails.", "a ping into Hivenet.",
...
//90 phrases
	var/static/list/starter_gossip = list(
		...
		"How interesting.", "You're curious?", "...you should know...", "Learned it myself.", "...yes[prob(50)?", but":".."].", "My thoughts?",
		"You don't get it.", "[prob(50)?"I":"We"] agree.", "The synchronicity [prob(50)?"said":"told us"]...", "Continuing on with the harmonization thread...",
		...
		"[prob(50)?"It":"This"] [prob(50)?"surfaced":"showed up"] in the Interstice.", "...ever since...", "[prob(50)?"Thanks to ":"Because of "] them...", "Wow.", "Uh.", "Huh?", "Excuse [prob(50)?"me":"us"].", "[prob(50)?"Hello":"Hi"][prob(50)?" there":""]?", "Nothing will [prob(50)?"happen":"come of this"].", "[prob(50)?"We":"I"] get it.",
		"[prob(50)?"I'm":"We're"] familiar with that.", "Well-said.", "Not a [prob(50)?"bad":"good"] observation.", "I hope [prob(50)?"I'm":"we're"] right about this.", "Is that so[prob(50)?"":" wrong"]?", "Why not?",
		"Enlightening!", "Truly, nothing compares.", "[prob(50)?"Truth":"Lies"]!", "[prob(50)?"Good":"Interesting"] insight.", "Let's synchronize [prob(50)?"this":"a thread"].", "[prob(50)?"What":"Quite"] a thread.", "What a revelation!",
		"You're [prob(50)?"not":""] alone[prob(50)?" on that one":""].", "Wait!", "Hold on...", "They're [prob(50)?"right":"wrong"].", "As far as [prob(50)?"I":"we"] know...", "Don't ruin it.", "You've had too much.", "[prob(50)?"Aha!":"Oh...."]",
		"[prob(50)?"Woah,":"That's"] too far.", "Slow down.", "Keep it up.", "[prob(50)?"I don't":"You won't"] believe this.", "So close, yet so far.", "There's always something.", "Meet up?",
		"Reporting.", "That's what I mean!", "It's [prob(50)?"exactly":"not"] what you think.", "Believe [prob(50)?"me":"us"].", "[prob(50)?"When I say it,":""] I [prob(50)?"really ":""]mean it.", "Get what [prob(50)?"I":"we"] mean?",
		"It's [prob(50)?"that":"about"] time again.", "Stop underestimating[prob(50)?" them":""]!", "[prob(50)?"You":"We"] cannot overestimate [prob(50)?"THIS":"them"].", "Queens!", "How [prob(50)?"is":"isn't"] there still doubt?", "It's [prob(50)?"finally ":""]clear now!"
		)
...
//140 phrases
	var/static/list/response_gossip = list(
		...
		"[prob(50)?"Nii'mii'katz":"The Dealership"] only pickpocket because Flagsdale was abandoned[prob(50)?" by Vaur":""].", "[prob(50)?"Well, ":""]Ak'riix is an ideology, [prob(50)?"not an organization":"it cannot be stopped"].", "Ak'riix will [prob(50)?"ruin":"destroy"] [prob(50)?"themselves":"everything"].",
		...
		"[prob(50)?"Tyria":"The Chapel of Queens"] is truly neutral ground", "[prob(50)?"Reopq":"Experience"] is truly a [prob(50)?"special":"one-of-a-kind"] subrealm.",
		...
		"Mouv is on the board of Einstein Engines.", "High Zo'ra [prob(50)?"overtook":"conquered"] all [prob(50)?"opposing":"the"] Hives in [prob(50)?"her birth region":"Xtykt'lotec Rift"].",
		...
		"K'lax [prob(50)?"should":"shouldn't"] stay vassals.", "Mother Restorationists are [prob(50)?"simply":"grossly"] defective.", "With Mother gone, K'lax should be led by [prob(50)?"all its Queens":"her direct descendant"].",
		...
		"[prob(50)?"C'thur":"C'thuric Warriors"] completely tupped the Lyukal in the [prob(50)?"Traverse":"Federation"].", "Every species will eventually [prob(50)?"invent":"be graced by"] the Aether.",
...
//35 phrases
	var/static/list/starter_happy = list (
		...
		"Our plans were accepted.", "We were picked[prob(50)?" for the project":""]!", "[prob(50)?"We":"I"] passed the interview.", "[prob(50)?"They're":"That's"] my apprentice.",
...
//35 phrases
	var/static/list/response_happy = list(
		"Wonderful!", "Praise the Queens.", "Good [prob(50)?"work":"on you"].", "How [prob(50)?"great":"amazing"].", "Try [prob(50)?"this":"my"] puzzlebox!", "[prob(50)?"We":"I"] have something for you",
		...
		"You may like this related realm[prob(50)?". Seems your taste":"Others have great reviews"].", "Persistent[prob(50)?" like Maa'krek":""]!", "You tupped!", "Keep tupping!", "Glad [prob(50)?"I":"We"] could assist.",
...
//40 phrases
	var/static/list/starter_tense = list(
		"Is it safe?", "What was that?", "I saw something.", "Did you feel that?", "I'm going to check [prob(50)?"it out":"on something"].",
		"Don't leave.", "It [prob(50)?"hurts!":"burns."]", "[prob(50)?"Queens":"BY THE QUEENS"]!", "Eshu'gat!", "Scourge!", "Help me.", "N-No!", "This can't be!",
		"Make it stop!", "STOP!", "The light...", "Something's...[prob(50)?"stuck ":""]inside[prob(50)?" me":".."].", "Nearing my limit here.", "Something's happening!", "Where's the k'ois when you need it?!",
		...
		"What IS this?", "This is a strange feeling.", "Did that just-", "[prob(50)?"I'm":"My body's"] giving out...", "Tired![prob(50)?" So...tired...":""]", "There's [prob(50)?"something":"someone"] out there.",
...
/datum/controller/subsystem/processing/HiveEchoes/proc/ToggleEchoes()
	...
	if(SSatlas.current_sector.catchecho == TRUE)
		...
		CHECKPLAYER
			if(isvaurca(player))
				CHECK_VALID_SOCKET
					to_chat(player, SPAN_CULT("The absence of typical echoes makes you feel dreadful."))
